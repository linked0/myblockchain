...
contract('ERC721Token', accounts => {
    ...
    describe('can transfer token', () => {
        ...
        it('only permissionusers can transfer tokens', async function() {
            let randomPersonTryingToSteamTokens = accounts[4]
            await expectThrow(this.contract.transferFrom(user1, randomPersonTryingToSteamTokens, tokenId, {from: randomPersonTryingToSteamTokens}))
        })
    })
